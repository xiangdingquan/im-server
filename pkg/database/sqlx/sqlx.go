...
func (db *DB) NamedQuery(c context.Context, query string, arg interface{}) (rows *Rows, err error) {
	idx := db.readIndex()
	for i := range db.read {
		if rows, err = db.read[(idx+i)%len(db.read)].namedQuery(c, query, arg); !ecode.EqualError(ecode.ServiceUnavailable, err) {
			return
		}
	}
...
func (db *DB) Select(c context.Context, dest interface{}, query string, args ...interface{}) (err error) {
	idx := db.readIndex()
	for i := range db.read {
		if err = db.read[(idx+i)%len(db.read)]._select(c, dest, query, args...); !ecode.EqualError(ecode.ServiceUnavailable, err) {
			return
		}
	}
...
func (db *DB) Get(c context.Context, dest interface{}, query string, args ...interface{}) (err error) {
	idx := db.readIndex()
	for i := range db.read {
		if err = db.read[(idx+i)%len(db.read)]._get(c, dest, query, args...); !ecode.EqualError(ecode.ServiceUnavailable, err) {
			return
		}
	}
