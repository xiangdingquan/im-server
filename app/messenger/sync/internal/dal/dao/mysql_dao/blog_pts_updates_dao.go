...
func (dao *BlogPtsUpdatesDAO) SelectByGtPts(ctx context.Context, user_id int32, pts int32) (rList []dataobject.BlogPtsUpdatesDO, err error) {
	var (
		query = "select user_id, pts, pts_count, update_type, update_data from blog_pts_updates where user_id = ? and pts > ? order by pts asc"
		rows  *sqlx.Rows
	)
