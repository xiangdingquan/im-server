...
func (s *Service) ChannelsTogglePreHistoryHidden(ctx context.Context, request *mtproto.TLChannelsTogglePreHistoryHidden) (*mtproto.Updates, error) {
	...
	log.Debugf("channels.togglePreHistoryHidden - metadata: %s, request: %s", md.DebugString(), request.DebugString())

	if md.IsBot {
		err := mtproto.ErrBotMethodInvalid
		log.Errorf("channels.togglePreHistoryHidden - error: %v", err)
		return nil, err
	}
	...
	channel, err := s.ChannelFacade.TogglePreHistoryHidden(ctx, request.Channel.ChannelId, md.UserId, mtproto.FromBool(request.Enabled))
	if err != nil {
		log.Errorf("channels.togglePreHistoryHidden - error: %v", err)
		return nil, err
	}
	...
	log.Debugf("channels.togglePreHistoryHidden - reply: {%s}", result.DebugString())
