package service

import (
	"context"

	"open.chat/mtproto"
	"open.chat/pkg/crypto"
	"open.chat/pkg/grpc_util"
	"open.chat/pkg/log"
)

var (
	dh2048P = []byte{
		,
	}

	dh2048G = int32(3) // []byte{0x03}
)

func (s *Service) MessagesGetDhConfig(ctx context.Context, request *mtproto.TLMessagesGetDhConfig) (*mtproto.Messages_DhConfig, error) {
	md := grpc_util.RpcMetadataFromIncoming(ctx)
	log.Debugf("messages.getDhConfig - metadata: %s, request: %s", md.DebugString(), request.DebugString())

	// 400	BOT_METHOD_INVALID	This method can't be used by a bot
	if md.IsBot {
		err := mtproto.ErrBotMethodInvalid
		log.Errorf("messages.getDhConfig - error: %v", err)
		return nil, err
	}

	// 400	RANDOM_LENGTH_INVALID	Random length invalid
	rl := request.RandomLength
	if rl == 0 {
		rl = 256
	}
	if rl != 256 {
		err := mtproto.ErrRandomLengthInvalid
		log.Errorf("messages.getDhConfig - error: %v", err)
		return nil, err
	}

	var dhConfig *mtproto.Messages_DhConfig

	if request.Version == 0 {
		dhConfig = mtproto.MakeTLMessagesDhConfig(&mtproto.Messages_DhConfig{
			G:       dh2048G,
			P:       dh2048P,
			Version: 3,
			Random:  crypto.RandomBytes(int(rl)),
		}).To_Messages_DhConfig()
	} else {
		dhConfig = mtproto.MakeTLMessagesDhConfig(&mtproto.Messages_DhConfig{
			G:       dh2048G,
			P:       dh2048P,
			Version: 3,
			Random:  crypto.RandomBytes(int(rl)),
		}).To_Messages_DhConfig()
	}

	log.Debugf("messages.getDhConfig - reply {%s}", dhConfig.DebugString())
	return dhConfig, nil
}
