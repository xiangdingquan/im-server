...
import (
	...
	"github.com/go-kratos/kratos/pkg/net/netutil"
	"github.com/go-kratos/kratos/pkg/net/trace"
	xtime "github.com/go-kratos/kratos/pkg/time"

	"open.chat/app/infra/databus/pkg/cache/redis"
	"open.chat/app/infra/databus/pkg/stat/prom"
	"open.chat/pkg/log"
)
...
func (d *Databus) dialInstance() (redis.Conn, error) {
	if instances, ok := d.dis.Fetch(context.Background()); ok {
		ins, ok := instances.Instances[env.Zone]
		if !ok || len(ins) == 0 {
			for _, is := range instances.Instances {
				ins = append(ins, is...)
			}
		}
		if len(ins) > 0 {
...
func (d *Databus) disc() {
	...
	if instances, ok := d.dis.Fetch(context.Background()); ok {
		if ins, ok := instances.Instances[env.Zone]; ok && len(ins) > 0 {
