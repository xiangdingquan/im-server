...
func (dao *ScheduledMessagesDAO) SelectByRandomId(ctx context.Context, user_id int32, random_id int64) (rValue *dataobject.ScheduledMessagesDO, err error) {
	var (
		query = "select id, user_id, user_message_box_id, peer_type, peer_id, dialog_id, random_id, message_type, message_data_type, message_data, message_box_type, scheduled_date, date2, state from scheduled_messages where user_id = ? and random_id = ? and state = 0 order by date2 desc limit 1"
		rows  *sqlx.Rows
	)
	rows, err = dao.db.Query(ctx, query, user_id, random_id)
