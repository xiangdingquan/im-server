...
func init() {
	gNewAlgo = &mtproto.PasswordKdfAlgo{
		Constructor: mtproto.CRC32_passwordKdfAlgoModPow,
		Salt1:       gNewAlgoSalt1,
		Salt2:       gNewAlgoSalt2,
		G:           gNewAlgoG,
		P:           gNewAlgoP,
	...
	gNewSecureAlgo = &mtproto.SecurePasswordKdfAlgo{
		Constructor: mtproto.CRC32_securePasswordKdfAlgoPBKDF2,
...
func (m *AccountCore) MakePasswordData(ctx context.Context, userId int32) (*passwordData, error) {
	var (
		err   error
		pData *passwordData
	)

	do, err := m.UserPasswordsDAO.SelectByUserId(ctx, userId)
	if err != nil {
...
func (p *passwordData) GetPassword() *mtproto.Account_Password {
	...
	case kStateEmailUnconfirmedPassword:
		noPassword := mtproto.MakeTLAccountPassword(&mtproto.Account_Password{
			EmailUnconfirmedPattern_FLAGSTRING: &types.StringValue{Value: makeEMailPattern(p.email)},
			NewAlgo:                            gNewAlgo,
			NewSecureAlgo:                      gNewSecureAlgo,
			SecureRandom:                       crypto.RandomBytes(256),
		})
	...
	case kStateConfirmedPassword:
		...
		if p.hint != "" {
			password.SetHint_FLAGSTRING(&types.StringValue{Value: p.hint})
		}
		return password.To_Account_Password()
	default:
		// bug.
...
func (p *passwordData) UpdatePasswordSetting(ctx context.Context, password *mtproto.InputCheckPasswordSRP, newSettings *mtproto.Account_PasswordInputSettings) error {
	var err error
	...
	case kStatePasswordNone:
		...
		if len(newSettings.NewPasswordHash) == 0 {
			err = mtproto.NewRpcError2(mtproto.TLRpcErrorCodes_PASSWORD_HASH_INVALID)
			log.Error("%v: current_password_hash need empty.", err)
			return err
		}
		...
		_, _, err = p.AccountCore.UserPasswordsDAO.InsertOrUpdate(ctx, do)
	...
	case kStateConfirmedPassword, kStateNoRecoveryPassword:
		...
		if len(newSettings.NewPasswordHash) > 0 {
			...
			_, err = p.UserPasswordsDAO.Update(ctx, map[string]interface{}{
...
func (p *passwordData) GetPasswordSetting(password *mtproto.InputCheckPasswordSRP) (*mtproto.Account_PasswordSettings, error) {
	...
	setting := mtproto.MakeTLAccountPasswordSettings(&mtproto.Account_PasswordSettings{
		Email_FLAGSTRING: &types.StringValue{Value: p.email},
	})
...
func (p *passwordData) RecoverPassword(ctx context.Context, code string) error {
	if p.Code != code {
...
func (m *AccountCore) CheckRecoverCode(ctx context.Context, userId int32, code string) error {
...
func (p *passwordData) clearPassword(ctx context.Context) error {
	_, err := p.UserPasswordsDAO.Update(ctx, map[string]interface{}{
		"new_algo_salt1": "",
		"V":              "",
		"srp_id":         0,
