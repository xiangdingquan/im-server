...
func (cs *customCodecServer) React(frame interface{}, c gnet.Conn) (out interface{}, action gnet.Action) {
	fmt.Println("frame:", string(frame.([]byte)))
...
func testCustomCodecServe(addr string, multicore, async bool, codec gnet.ICodec) {
	...
	err = gnet.Serve(cs, []string{addr}, gnet.WithMulticore(multicore), gnet.WithTCPKeepAlive(time.Minute*5), gnet.WithCodec(codec))
	if err != nil {
		panic(err)
	}
}
