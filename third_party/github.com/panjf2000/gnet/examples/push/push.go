...
func (ps *pushServer) OnInitComplete(srv gnet.Server) (action gnet.Action) {
	log.Printf("Push server is listening on %s (multi-cores: %t, loops: %d), "+
		"pushing data every %s ...\n", srv.AddrsString(), srv.Multicore, srv.NumEventLoop, ps.tick.String())
	return
}
...
func (ps *pushServer) React(frame interface{}, c gnet.Conn) (out interface{}, action gnet.Action) {
	out = frame
	return
}
...
func main() {
	...
	log.Fatal(gnet.Serve(push, []string{fmt.Sprintf("tcp://:%d", port)}, gnet.WithMulticore(multicore), gnet.WithTicker(ticker)))
}
